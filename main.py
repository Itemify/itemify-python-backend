...
# The structure in our s3 bucket is as follows:
# <item_id>/model.(stl|obj)
# <item_id>/img/<image_name>.(png|jpg|jpeg|webp) -> generated by thumbnail-consumer and uploaded by users
# <item_id>/<model_name>.gcode -> generated by cura api # TODO: CHANGE TO KAFKA
@app.post("/files/")
async def create_file(file: UploadFile, is_image: bool = Form(...), model_id: int = Form(...), token=Depends(get_auth)):
    if is_image:
        ...
        out_Model_Name = f"{model_id}_{file.filename}"

        # Save the file to the disk
        with open(out_Model_Name, "wb+") as file_object:
            file_object.write(file.file.read())

        # Save the file to the storage (commonly digital ocean s3)
        with open(out_Model_Name, "rb+") as file_object:
            storage.storeNormalFile(filename, file_object)

        if model_exist:
            cursor.execute(SQL_EXEC_INSERT_ITEM, (model_id, 
                                                  file.filename, filename, 
                                                  str(model_id) + "/img/" + file.filename + ".png"))
            conn.commit()

    if not is_image and model_exist:
        # We want to trigger some events on the upload of a new model
        # These are commonly time-consuming tasks, so we want to do them async
        # We use kafka to trigger the tasks
        kafka_producer.send('itemify-item-upload', {
            'item_file': {
                "item_id": model_id, "file_path": filename, "file_name": file.filename
            }
...
def calculate_size(item: Item, token=Depends(get_auth)):
    kafka_producer.send('itemify-item-upload', {
        'item_file': {
            "item_id": item.id, "file_path": item.file_path, "file_name": item.file_name
        }
    }).add_callback(on_send_success) \
        .add_errback(on_send_error)
